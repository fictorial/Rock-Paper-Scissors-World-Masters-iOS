...
+ (id)sceneWithLayerClass:(NSString *)layerClassName {
    CCScene *scene = [BaseScene node];
    [scene addChild:[NSClassFromString(layerClassName) node]];
    return scene;
}

- (id)init {
    if ((self = [super init]))
        [self addChild:[BackgroundLayer node] z:-1 tag:kRPSTagLayerBackground];
    return self;
}
